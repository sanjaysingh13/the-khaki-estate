...
from .models import (
    Resident, Announcement, AnnouncementCategory, AnnouncementRead, Comment,
    MaintenanceRequest, MaintenanceCategory, MaintenanceUpdate,
    CommonArea, Booking, Event, EventRSVP, MarketplaceItem,
    Document, EmergencyContact, Notification, NotificationType
)
...
def announcement_list(request):
    ...
    """
    announcements = Announcement.objects.all().order_by('-is_pinned', '-is_urgent', '-created_at')
    ...
    if read_status == 'unread':
        read_announcements = AnnouncementRead.objects.filter(resident=request.user).values_list('announcement_id', flat=True)
...
def announcement_detail(request, announcement_id):
    ...
    comments = Comment.objects.filter(announcement=announcement, parent=None).order_by('created_at')
    ...
    if is_committee_member(request.user):
        ...
        read_stats = {
            ...
            'read_percentage': (read_count / total_residents * 100) if total_residents > 0 else 0
        }
...
def announcement_edit(request, announcement_id):
    ...
    if request.method == 'POST':
        ...
        valid_until = request.POST.get('valid_until')
        announcement.valid_until = datetime.fromisoformat(valid_until) if valid_until else None
...
def add_comment(request, announcement_id):
    ...
    if not content:
        return JsonResponse({'status': 'error', 'message': 'Comment content is required'})
...
def maintenance_request_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, f'Maintenance request {maintenance_request.ticket_number} created successfully!')
            return redirect('backend:maintenance_detail', request_id=maintenance_request.id)
...
def update_maintenance_status(request, request_id):
    ...
    try:
        ...
        if assigned_to_id:
            assigned_user = Resident.objects.get(id=assigned_to_id, is_committee_member=True)
...
def add_maintenance_update(request, request_id):
    ...
    content = request.POST.get('content')
    if not content:
        return JsonResponse({'status': 'error', 'message': 'Update content is required'})
    ...
    try:
        ...
        if is_committee_member(request.user):
            # Committee member added update, notify resident
            # TODO: Implement notification sending
            pass
        else:
            # Resident added update, notify committee
            # TODO: Implement notification sending
            pass
...
def booking_list(request):
    ...
    """
    bookings = Booking.objects.filter(booking_date__gte=timezone.now().date()).order_by('booking_date', 'start_time')
...
def update_booking_status(request, booking_id):
    ...
    try:
        ...
        return JsonResponse({'status': 'success', 'message': 'Booking status updated successfully'})
...
def event_detail(request, event_id):
    ...
    total_attendees = sum(rsvp.guests_count + 1 for rsvp in rsvps if rsvp.response == 'yes')
    ...
    context = {
        ...
        'can_edit': is_committee_member(request.user) or event.organizer == request.user,
    }
...
def event_create(request):
    ...
    if request.method == 'POST':
        ...
        # Validate required fields
        if not all([title, description, event_type, start_datetime, end_datetime, location]):
            messages.error(request, 'Please fill in all required fields.')
...
def marketplace_update_status(request, item_id):
    ...
    try:
        ...
        return JsonResponse({'status': 'success', 'message': 'Item status updated successfully'})
